...
- 初始化 `docs/`、`assets/`、`pages/`、`logs/`、`pages.json`，并为产品项目初始化本地 Git 仓库。
- 将规划文档、UI 图片、生成素材、切图区域和页面元信息保存在用户选择的项目目录内。
- 将当前 AI 产品项目导出为 ZIP，包含核心项目数据：`pages.json`、`pages/`、`assets/`、`docs/`。
...
- 支持将 `page-plan.md` 同步回项目页面索引，让后续 UI 生成使用最新页面列表和页面描述。

### 页面设计与资产提取

![页面生成](./docs/imgs/页面生成.png)

- 根据轻量项目索引、页面级数据文件和用户提示词，为每个页面生成 UI 概念图。
- 页面图片按版本保存在本地，可以切换当前激活版本，不覆盖历史结果。
- 支持在已有 UI 图片上圈选批注，将批注内容和参考图一起交给 Codex，重新生成新的页面版本。
- 支持提取干净可复用的页面背景，并将背景路径写入当前页面数据文件。
...
- `pages`：页面 ID、路由、摘要、更新时间和每个页面的 `dataDir`。

每个 `pages/<pageId>/` 文件夹保存页面级数据：

- `page.json`：完整页面提示词、当前图片版本、背景图和更新状态。
- `assets.json`：已生成素材，包含稳定 `id`、名称、描述、路径和来源选区。
- `slice-selections.json`：用户或 AI 识别出的切图区域。

### 为什么拆分页面数据？

早期项目把所有页面详情、切图区域和素材元数据都放在一个大的 `pages.json` 中。这个方式容易检查，但项目变大后，页面级任务会让 Codex 读入太多无关上下文。拆分后仍然保持人类可读，同时减少 token 消耗，降低并发页面任务互相覆盖的概率，也让 Git diff 更聚焦在发生变化的页面上。

旧版 v1 项目仍可打开。首次写入时，cxDesinger 会迁移到拆分结构，并在项目根目录保留 `pages.v1.backup.json` 备份文件。
