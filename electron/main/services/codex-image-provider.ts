...
export class CodexImageProvider {
  ...
  private createSliceIdentificationPrompt(params: IdentifySliceSelectionsParams): string {
    ...
    return [
      ...
      "- name 使用简短、明确、可索引的名称，必须贴合当前项目领域，例如“上传图片按钮”“分析进度卡片”“结果风险标签”。",
      "- description 说明该素材的视觉内容、用途、状态和边界注意事项，方便用户确认和后续 AI 切图。",
      "- 返回示例：{\"selections\":[{\"name\":\"上传图片按钮\",\"description\":\"主操作按钮，包含上传图标与按钮文字，需要保留边框、阴影和点击态视觉边界。\",\"selection\":{\"x\":100,\"y\":120,\"width\":320,\"height\":180}}]}。"
    ].join("\n");
  }
  ...
  private createPageBackgroundPrompt(params: GeneratePageBackgroundParams): string {
    ...
    return [
      ...
      "- 输出画幅和构图应匹配参考图与当前项目规划；不要引入与项目无关的设备或业务设定。",
      "- 保存到目标输出路径，并返回 {\"path\":\"目标输出路径\"}。"
    ].join("\n");
  }
  ...
  private createSliceAssetsPrompt(params: GenerateSliceAssetsParams): string {
    ...
    return [
      ...
      "- 优先生成单张 4096x4096 洋红幕 sprite sheet：背景使用纯色 #ff00ff，所有未使用区域都必须保持该 key 色；如果放不下，再拆成多张 sprite sheet。不要回退成每个区域一张独立生图。",
      "- manifest 必须包含 keyColor=\"#ff00ff\"，并为每个素材写入 cellRect，而不是把 spriteRect 当作最终精裁坐标。cellRect 用于粗分配，最终由 split_chroma_sprite.py 通过 border-connected chroma key + 连通域自动紧裁。",
      "- 每个组件必须完整位于自己的 cellRect 内，并与其他组件至少间隔 96px key 色背景；不要渲染 cell 编号、辅助线、边框或标签。",
      "- 不要在组件内部使用 #ff00ff 或接近 #ff00ff 的洋红色；如果组件包含粉色或珊瑚色装饰，必须让它与外部 key 背景不连通。",
      "- 必须运行 split_chroma_sprite.py，而不是 split_sprite_sheet.py；该脚本只删除从雪碧图外边界连通进入的 key 背景，避免误删组件内部相近颜色。",
      ...
      "- name 使用简短、明确、可索引的名称，必须根据当前项目领域和框选内容命名，例如“上传图片按钮”“检测结果卡片”“风险百分比徽标”。",
